...
m.test=function (X1,X2){
  ...
  for(i in 1:100){
    ...
    temp2=ppp(allx[mix[(n1+1):tot]], ally[mix[(n1+1):tot]],
              window=owin(c(min(c(X1x,X2x)),max(c(X1x,X2x))),c(min(c(X1y,X2y)),max(c(X1y,X2y)))))
    differ=Mest(temp1,temp2,r=r)$Ld
    hold[i,]=differ
    M.dist[i]=sum(differ)
  }
  ...
  M.p=sum(M.dist>=sum(M.est$Ld))/length(M.dist)
  if (M.p==0){M.p=1/length(M.dist)}

  M.sum=sum(M.est$Ld)
  M.ci=quantile(M.dist,c(.025,.975))
  
  results=list(r=r,Mest=M.est$Ld,envelope=data.frame(Mean,Up,Down),p=M.p,M.sum=M.sum,ConfInt=M.ci)
  return(results)
}